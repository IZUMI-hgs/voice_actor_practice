class ResultDisplayService
  ...
  def call
    if result.score.ceil(2).zero? && result.quote.emotion == 'sad'
      {
    ...
    elsif result.score.ceil(2).zero? && result.quote.emotion == 'happy'
      {
        score: (result.created_at.strftime('%S')[1].to_i * 3.09).ceil(2),
        message: "もっと感情を出すことができるはず！殻を破って演技に全力投球してみて下さい！\nヒント：演技の方向性を変える事で判定結果が変わる場合があります。\n例:怒鳴る演技→声は小さいけど怒りをにじませる演技など・・"
      }
    elsif result.score.ceil(2).zero?
      {
      ...
      {
        score: result.score.ceil(2) + 15,
        message: "なかなか良い表現ですね！\n感情が良く出ています！あと一歩で高得点が狙えそうです！"
      }
    elsif result.score.ceil(2) >= 10 && @result.score.ceil(2) < 40
      {
        score: result.score.ceil(2),
